...
function MemberCard({ member }: { member: Member }) {
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[1.5rem] bg-white shadow-[0_8px_32px_rgba(0,0,0,0.07)] transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_18px_56px_rgba(0,0,0,0.13)]">
      <Link href={`/profiles/${member.slug}`} className="relative aspect-[5/6] overflow-hidden">
        <Image
          src={member.photo}
          alt={member.name}
          fill
        ...
        <span className="absolute bottom-4 left-4 rounded-full bg-white/90 px-3 py-1.5 text-[9px] uppercase tracking-[0.2em] text-charcoal">
          {member.category}
        </span>
      </Link>
      ...
      <div className="flex flex-1 flex-col justify-between p-5">
        <div>
          <h3 className="font-serif text-[1.2rem] font-light leading-snug text-charcoal">{member.name}</h3>
          <p className="mt-1 text-[11px] uppercase tracking-[0.16em] text-charcoal/45">{member.company}</p>
          <p className="mt-4 border-t border-charcoal/8 pt-4 text-[13px] leading-6 text-charcoal/65">
            {member.specialty}
          </p>
        </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <Link
            ...
            className="inline-flex items-center justify-center rounded-full bg-charcoal px-4 py-2.5 text-[10px] uppercase tracking-[0.18em] text-white transition-all duration-300 hover:bg-charcoal/85"
          >
            View Details
          </Link>
          {member.website && (
            <a
              ...
              className="inline-flex items-center justify-center rounded-full border border-charcoal/15 px-4 py-2.5 text-[10px] uppercase tracking-[0.18em] text-charcoal transition-all duration-300 hover:border-charcoal hover:bg-charcoal hover:text-white"
            >
              Website
            </a>
          )}
